...
function App() {
  ...
  const [mensagemLoading, setMensagemLoading] = useState("Conectando aos servidores BF Dias...")

  useEffect(() => {
    const verificarServidor = async () => {
      try {
        // Tenta bater na rota de saúde do seu Python
        // Quando estiver no ar, mude para a URL do Render
        await axios.get('http://localhost:8000/health') 
        setEstaConectado(true)
      } catch (error) {
        console.log("Servidor em standby... Tentando novamente.")
        // Se demorar muito, você pode trocar a mensagem para o usuário
        setTimeout(() => setMensagemLoading("O servidor está acordando, quase lá..."), 15000)
          ...
          <header className="bg-white shadow-sm py-6">
            <div className="max-w-6xl mx-auto px-4 flex justify-between items-center">
              <h1 className="text-xl font-black text-blue-900 uppercase tracking-tighter">
                BF Dias <span className="font-light text-slate-400">| Engineering</span>
              </h1>
